...
def test_parser():
    ...
    devices = find_devices_by_primitive(
        ...
        primitives=["INPUT", "OUTPUT_OR2", "OUTPUT_BUFFER", "NOT", "NOR2"],
    ...
    print(f"NOT devices count: {len(storage.not_nor2_devices)}")
    
    print("\nInput devices in storage:")
    for device_id, info in storage.input_devices.items():
        print(f"ID: {device_id}, Name: {info['name']}") #, Color: {info['color']}, Regulator: {info['regulator']}")
    
    print("\nOutput devices in storage:")
    for device_id, info in storage.output_devices.items():
        print(f"ID: {device_id}, Name: {info['name']}") #, Color: {info['color']}, Regulator: {info['regulator']}")
        
    print("\nNOT_NOR2 devices in storage:")
    for device_id, info in storage.not_nor2_devices.items():
        print(f"ID: {device_id}, Name: {info['name']}, Color: {info['color']}, Regulator: {info['regulator']}")
