...
def find_devices_by_primitive(json_path: str, 
    ...
    try:
        ...
        print(f"Error parsing JSON file: {repr(e)}")
    return []

def update_storage_with_devices(json_path: str) -> None:
    """Find input/output/NOT/NOR2 devices and update storage with essential information"""
    devices = find_devices_by_primitive(
        json_path=json_path,
        primitives=["INPUT", "OUTPUT_OR2", "OUTPUT_BUFFER", "NOT", "NOR2"],
        fields_to_extract=["name", "color", "identifier", "primitive_identifier", "regulator"]
    )
